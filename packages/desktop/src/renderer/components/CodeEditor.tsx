...
export default function CodeEditor({ value, onChange, readOnly = false }: CodeEditorProps): React.ReactElement {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const { line } = (e as CustomEvent).detail as { file: string; line: number }
      const view = viewRef.current
      if (!view || !line) return
    ...
    window.addEventListener('openags-scroll-to-line', handler)
    return () => window.removeEventListener('openags-scroll-to-line', handler)
  }, [])
