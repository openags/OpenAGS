      ...
      for (const line of raw.split(/\r?\n/)) {
        ...
        try {
          ...
          if (event.type === 'tool_use') {
            writer.sendToolUse(event.tool_name || event.name || 'tool', event.parameters || event.input || {})
          }
          ...
          if (event.type === 'result') {
            ...
            writer.sendResult(event.cost_usd, {
              input: stats.input_tokens || event.input_tokens || 0,
              output: stats.output_tokens || event.output_tokens || 0,
            })
          }
        } catch {
