...
export default function ProposalEditor({ projectId, projectName }: Props): React.ReactElement {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/manuscript/${projectId}/pdf/main.pdf?${moduleParam}&t=${Date.now()}`)
      .then(resp => {
  ...
  const openFile = async (path: string, name: string) => {
    ...
    if (name.endsWith('.pdf')) {
      try {
        ...
        const resp = await fetch(`${BASE_URL}/api/manuscript/${projectId}/pdf/${path}?${moduleParam}&t=${Date.now()}`)
        if (resp.ok) {
    ...
    if (!(path in contents)) {
      try {
        const fc = await api.get<{ content: string }>(`/api/manuscript/${projectId}/file?path=${encodeURIComponent(path)}&${moduleParam}`)
  ...
  const saveFile = async (path: string) => {
    ...
    try {
      await api.put(`/api/manuscript/${projectId}/file?${moduleParam}`, { path, content })
    ...
    try {
      ...
      if (result.success && result.pdf_path) {
        ...
        const pdfResp = await fetch(
          `${BASE_URL}/api/manuscript/${projectId}/pdf/${result.pdf_path}?${moduleParam}&t=${Date.now()}`
        )
